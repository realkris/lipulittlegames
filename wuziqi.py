...
QUIT = Text(Point(500,20),"Quit")
QUIT.setFill('red')
RESTART = Text(Point(500,60),"Replay")
RESTART.setFill('red')
#数据初始化，把棋盘上的棋子和提示清空
def init():
    ...
    for i in range(len(list)):
        list[-1].undraw()
        list.pop(-1)
    aiFirst.setText("AI First")
    manFirst.setText("I'm First")
    notice.setText("")
    last_ai.setText("")
    last_man.setText("")
#画棋盘
...
def go(x,y):
    ...
    if(ban(x,y)):
        ...
        if(start==ai):
            notice.setText("AI wins!\nRestart")
        else:
            notice.setText("Gamer wins!\nRestart")
##是否重新开始游戏
def Restart(p):
    global RESTART_FLAG
    x=p.getX(); y=p.getY()
...
def whoStart(p):
    ...
    if((abs(500-x)<40) and (abs(100-y)<15)): #AI 先手
        start=1
        go_first=1
        aiFirst.setText("AI is Black")
        manFirst.setText("You are White")
        return True
    elif((abs(500-x)<40) and (abs(140-y)<15)): #玩家先手
        start=2
        go_first=2
        aiFirst.setText("AI is white")
        manFirst.setText("You are black")
        return True
    else:
        return False
##------------------------------------------------------------------##
...
if __name__=='__main__':
    ...
    while(not is_end):
        RESTART_FLAG=False
        if(start==ai):
            notice.setText("AI Hand...")
            AI1()
        else:
            notice.setText("Your Hand...")
            manPlay()
        start=3-start
        if(RESTART_FLAG):
            notice.setText("Who Go First")
